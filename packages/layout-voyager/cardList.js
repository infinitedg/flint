...
/**
Subtemplate to layout_voyager
@class layout_voyager_cardList
*/
...
Template.layout_voyager_cardList.created = function() {
  Router.onAfterAction(function() {
    $('.card-area').fadeIn();
  }, {
    name: 'layout_voyager_fading'
  });
};

Template.layout_voyager_cardList.destroyed = function() {
  // Remove hook for fading in after a template has loaded
  
  // Find the element
  var h = _.find(Router._globalHooks.onAfterAction, function(hook){
    return (hook.options.name === 'layout_voyager_fading');
  });
  // Get its index
