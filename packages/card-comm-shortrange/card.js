...
Template.card_shortRangeComm.amplitudeValue = function () {
  ...
  var time = (new Date()).getTime();
  animate(canvas, theta, time);
  return Flint.collection('systems').find({'simulatorId': Flint.simulatorId(), 'name': Comm}).fetch()[0].commAmplitude;
};
...
context.beginPath();
context.moveTo(0, 0);
var f = parseInt(Flint.system('Short Range Communications', 'commFrequency')) / 16; //parseInt($('#frequency').val())/8;
  ...
  this.conditionObserver = Flint.collection('systems').find({'simulatorId': Flint.simulatorId(), 'name': Comm}).observeChanges({
    added: function (id, fields) {
      if (fields.commFrequency && fields.commAmplitude){
        $('#frequency').slider();
        $('#amplitude').slider();
        $('#frequency').slider('setValue',parseInt(fields.commFrequency));
        $('#amplitude').slider('setValue',parseInt(fields.commAmplitude));
      }
      if (fields.commHail) {
    ...
    changed: function (id, fields) {
      ...
      if (fields.commHail) {
        ...
        canvas = document.getElementById('canvasCandy');
        var theta = 0;
        var time = (new Date()).getTime();
        animate(canvas, theta, time);
      }
