Template.card_lightingCircuit.rendered = function() {
	...
	shrinker.style.zoom = 1;
	window.addEventListener("mousewheel", MouseWheelHandler, false);

	function MouseWheelHandler(e) {
		var delta = Math.max(-1, Math.min(1, (e.wheelDelta || -e.detail)));
		shrinker.style.zoom = Math.max(0.2, Math.min(1, parseFloat(shrinker.style.zoom) + (0.05 * delta)));
		updateConnectors();
	}
	$('.dropdown-toggle').dropdown();

	//Panning function
	function startDraggingBackground(event) {
		var el;
		var x, y;
		...
		document.getElementById('svgCanvas').addEventListener("mousemove", whileDraggingBackground, true);
		document.getElementById('svgCanvas').addEventListener("mouseup", stopDraggingBackground, true);
	...
	function whileDraggingBackground(event) {
		var x, y;
		$('#modules').children().each(function(c, e) {
			...
			e.style.left = (parseInt(e.style.left, 10) + x - dragObj.cursorStartX) + "px";
			e.style.top = (parseInt(e.style.top, 10) + y - dragObj.cursorStartY) + "px";
		})
		dragObj.cursorStartX = x;
		dragObj.cursorStartY = y;
		updateConnectors();
	...
	function stopDraggingBackground(event) {
		document.getElementById('svgCanvas').removeEventListener("mousemove", whileDraggingBackground, true);
		document.getElementById('svgCanvas').removeEventListener("mouseup", stopDraggingBackground, true);
	}
